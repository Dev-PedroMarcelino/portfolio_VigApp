...
import { Check, Plus, Palette, Ruler } from "lucide-react";
import type { CartLine, Vista360Content } from "./content";
import { VISTA_COLORWAYS, VISTA_PRODUCT } from "./content";
import { ProductStage } from "./product-stage";
import { Kicker, Chip, money, shot } from "./ui";

/**
 * The featured pair: one shoe on a type turntable, graded live into three
 * colorways, inside the store's neon house frame. The buy panel feeds the same
 * mini-cart as the rest of the store.
 */
export function Vista360({
  ...
  const add = () => {
    ...
    onAdd({
      ...
      size,
      price: content.price,
      image: shot(VISTA_PRODUCT.photo, 400),
    });
    setJustAdded(true);
      ...
      <div className="relative mx-auto max-w-6xl">
        ...
        <div className="mt-12 grid items-start gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <motion.div
            ...
            className="relative"
          >
            <div className="relative rounded-[2rem] border border-[var(--d-accent)]/50 bg-[var(--d-panel)] p-5 shadow-[0_0_60px_rgba(255,61,129,0.18)] sm:p-7">
              {/* corner ticks */}
              ...
              <ProductStage
                photo={VISTA_PRODUCT.photo}
                alt={content.photoAlt}
                ringText={content.ringText}
                colorways={VISTA_COLORWAYS}
                pins={content.pins}
                swatchLabel={content.swatchLabel}
              />
            </div>
            ...
            <p className="mt-4 flex items-center gap-2 text-[0.62rem] font-bold uppercase tracking-[0.16em] text-[var(--d-mute)] [font-family:var(--demo-body)]">
              <Palette className="h-3.5 w-3.5 text-[var(--d-accent)]" strokeWidth={2} aria-hidden />
              {content.featureNote}
            </p>
          </motion.div>
